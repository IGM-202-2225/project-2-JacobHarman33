...
public class Pirate : Agent
{
    ...
    public float seekScalar = 1.5f;
    public float allyScalar = 2f;
    ...
    public PirateLeader pirateLeader;
    public CollisionManager collisionManager;

    // Start is called before the first frame update
    void Start()
    {
        pirateLeader = FindObjectOfType<PirateLeader>();
    ...
    public override void CalculateSteeringForces()
    {
        if (Time.time > wanderTime)
        {
            ...
            totalForce += Seperate(AgentManager.Instance.Pirates);
            if (foundLeader == false)
            {
                totalForce += Seek(pirateLeader.transform.position) * seekScalar;
                totalForce += Pursue(AgentManager.Instance.Allies) * allyScalar;
                FindLeader(collisionManager.collidableObjects);
            }
            else
            {
                totalForce += Pursue(AgentManager.Instance.Allies) * (allyScalar + 1);
            }
